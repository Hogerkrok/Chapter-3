...
function estimateTotal(event) {
	...
	var itemBball = parseInt(document.getElementById('txt-q-bball').value, 10) || 0,
		...
		itemSport = parseInt(document.getElementById('txt-q-sport').value, 10) || 0,
		shippingState = state.value,
		shippingMethod = document.querySelector('[name=r_method]:checked').value || "";
		
	var totalQty = itemBball + itemJersey + itemPower + itemSport,
		shippingCostPer,
		shippingCost,
		taxFactor = 1,
		estimate,
		totalItemPrice = (90 * itemBball) + (25 * itemJersey) + (30 * itemPower) + (4 * itemSport);
